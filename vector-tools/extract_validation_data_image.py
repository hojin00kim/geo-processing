...
def compute_levels(index_df):
    """
    Find maximum and minimum values and find values corresponding user defined levels
    """
...
def find_nearest(array, value):
    """
    Function to find closest number with given values from an array
    """
...
def find_closest_index(val_list, rounded_levels):
    """
    Find closest values from what was divided
    """
    # convert the list of lai values into an array
    data_array = np.asarray(val_list)

    # find closest numbers with the levels
    findings = []
    for val in rounded_levels:
        n = round(find_nearest(data_array, val), 2)
        findings.append(n)
...
def make_image_filename(validation_df):
    """
    Craft image file name based on Range/Column ID from validation data
    """
    df = validation_df
...
def main(root_dir, infile, product):
    ...
    df_temp = create_output_df(findings, df)

    # save datafrme with unique values (dropping duplicate)
    df_validation = df_temp.drop_duplicates(subset = ['Value'])

    # save validation information to an excel file
    outstr = infile.split('.')[0]
    outname = outstr + '_validation.csv'
    df_validation.to_csv(outname, index = False)

    # copy extracted image files for validation into a folder
    plot_image_dir = os.path.join(root_dir, 'PlotImages')
    val_image_dir = os.path.join(root_dir, 'ValidationImages')

    if not os.path.exists(val_image_dir):
        os.makedirs(val_image_dir)

    copy_image_files(plot_image_dir, val_image_dir, df_validation)
